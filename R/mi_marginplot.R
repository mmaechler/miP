             ...
             ann = par("ann"), axes = TRUE, frame.plot = axes, ...)
{
    ...
    breit <- d[2]
    if(length(x) != 2)
        stop("'x' must be of length 2, for selecting to columns")
    var1_nr <- x[1]
    var2_nr <- x[2]
    if(is.character(sel <- c(var1_nr,var2_nr)) ## two integer indices are ok
       && !all(ii <- sel %in% colnames(ds)))
	stop("invalid column name(s) in 'x': ", paste(sel[!ii], collapse=", "))
    x <- ds[, sel]
#### MI Daten bekommen
    mis_data1 <- 0
    for(i in 1:breit)
...
####
    ...
    localPoints(x[, 1], x[, 2], cex = cex, col = colalpha[1],
                pch <- pch[1], ...)
    lines(lowess(x[,1][!is.na(x[,1])][!is.na(x[,2][!is.na(x[,1])])],
                 x[,2][!is.na(x[,1])][!is.na(x[,2][!is.na(x[,1])])]), col = "black", lty = 2)
    if(leg == TRUE)
    {
    ...
    if (any(!is.na(x[!miss[, 2], 1]))) {
        ...
        boxplot(xbox, boxwex = 0.4, col = if (fillbox) col[1],
                horizontal = TRUE, add = TRUE, at = 0.5,
                axes = FALSE)
    }
    if (any(!is.na(x[miss[, 2], 1]))) {
        xbox <- x[miss[, 2], 1]
    ...
    if (isTRUE(ann)) {
        localTitle <- function(..., cex.text, log, col, bg, pch, cex, lty, lwd)
            title(...)
